...
ZIP_DIR="$REPO_ROOT/ZIP final/Linux"
OUT_DIR="$REPO_ROOT/dist/CUMA_linux"
ZIP_PATH="$ZIP_DIR/CUMA_linux.tar.gz"
VENV_PY="$REPO_ROOT/.venv/bin/python"
APP_VERSION="1.100.37"

fail() {
  echo
  echo "[FALHA] A compilação Linux não foi concluída." >&2
  exit 1
}
trap fail ERR

echo "============================================================"
echo "CUMA - COMPILAÇÃO LINUX"
echo "Repositório: $REPO_ROOT"
echo "Saída:       $ZIP_PATH"
echo "============================================================"

for required in cuma.py cuma_updater.py requirements.txt cuma_linux.spec; do
  [[ -f "$REPO_ROOT/$required" ]] || { echo "[ERRO] Ausente: $REPO_ROOT/$required" >&2; exit 1; }
done

if [[ "${1:-}" == "--check" ]]; then
  echo "[OK] Estrutura Linux localizada corretamente."
  exit 0
fi
...
"$VENV_PY" -m PyInstaller --noconfirm "$REPO_ROOT/cuma_linux.spec"
...
rm -f "$ZIP_PATH"
tar -C "$REPO_ROOT/dist" -czf "$ZIP_PATH" CUMA_linux
...
if [[ "${CUMA_SKIP_MANIFEST:-0}" == "1" ]]; then
  ...
  "$VENV_PY" "$REPO_ROOT/scripts/preparar_manifesto_release.py" soldieg CUMA "$APP_VERSION" "$ZIP_PATH" Stable "$REPO_ROOT/NOTAS_RELEASE.md" linux || \
